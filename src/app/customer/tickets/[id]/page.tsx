...
export default function TicketDetailPage() {
  ...
  const hasDetails = ticket.category || ticket.severity || ticket.description || ticket.urgency || ticket.preferredContact;
  ...
  return (
    <div className="h-[calc(100vh-80px)] flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 flex items-center justify-between px-1 py-2 border-b border-[#6f8551]/10 bg-white/50">
        <Link href="/customer/tickets">
          <Button variant="ghost" size="sm" className="gap-1.5 text-muted-foreground hover:text-[#6f8551] h-8">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h1 className="font-semibold text-[#2D3E2F] truncate">{ticket.subject}</h1>
              </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {ticket.severity && (
                      <Badge variant="outline" className={cn(
                        ...
                        ticket.severity === "critical" ? "border-red-400 text-red-600" :
                        ticket.severity === "major" ? "border-orange-400 text-orange-600" : ""
                      )}>
                    ...
                    {ticket.urgency && (
                      <Badge variant="outline" className={cn(
                        "gap-1 text-[10px] h-5 px-1.5",
                        ticket.urgency === "immediate" ? "border-red-400 text-red-600" :
                        ticket.urgency === "high" ? "border-orange-400 text-orange-600" : ""
                      )}>
                    ...
                    {ticket.preferredContact && (
                      <Badge className="gap-1 text-[10px] h-5 px-1.5 bg-[#6f8551]/10 text-[#6f8551] hover:bg-[#6f8551]/20">
                        {ticket.preferredContact === "call" && <Phone className="h-2.5 w-2.5" />}
                        {ticket.preferredContact === "email" && <Mail className="h-2.5 w-2.5" />}
                        {ticket.preferredContact === "chat" && <MessageCircle className="h-2.5 w-2.5" />}
